"""Refresh generated toolchain files."""
...
def _pin(source, name, digest):
    pattern = rf'("{re.escape(name)}",\n\s+[^,]+,\n\s+")[0-9a-f]{{64}}'
    updated, count = re.subn(pattern, rf"\g<1>{digest}", source)
    if count != 1:
...
def _main():
    ...
    data = _pin(
        _pin(wrapper.read_text(), "manifest.json", sha256(manifest).hexdigest()),
        "scripts/cli.py",
        sha256(entrypoint.read_bytes()).hexdigest(),
    ).encode()
